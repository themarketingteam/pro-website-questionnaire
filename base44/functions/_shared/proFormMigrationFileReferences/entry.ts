export const PRO_FORM_FILE_REFERENCE_CLASSIFICATIONS = Object.freeze([
  'external_stable_url', 'public_base44_url', 'app_scoped_base44_asset',
  'signed_or_expiring_url', 's3_url', 'cloudfront_url', 'embedded_data_url',
  'unknown', 'missing',
]);

export function classifyProFormFileReference(value: unknown) {
  ...
  if ([...url.searchParams.keys()].some((key) => /^(?:x-amz-|signature|expires|token|policy|key-pair-id)/iu.test(key))) {
...
export function redactProFormFileReference(value: unknown) {
...
async function hostFingerprint(host: string | null) {
  if (!host) return null;
  const digest = await crypto.subtle.digest('SHA-256', new TextEncoder().encode(host));
  return [...new Uint8Array(digest)].map((byte) => byte.toString(16).padStart(2, '0')).join('').slice(0, 16);
}

export async function buildProFormFileReferenceAuditRecord(input: Record<string, unknown>) {
  const classification = classifyProFormFileReference(input.value);
  let host: string | null = null;
  try { host = new URL(String(input.value)).hostname.toLowerCase(); } catch { /* classified above */ }
  ...
  return Object.freeze({
    ...
    classification,
    hostFingerprint: await hostFingerprint(host),
    ...
    manualReview: blocking,
    redactedReference: redactProFormFileReference(input.value),
  });
}

export function collectProFormFileReferences(value: unknown, prefix = ''): Array<Record<string, unknown>> {
...
export function summarizeProFormFileReferenceAudit(records: ReadonlyArray<Record<string, unknown>>) {
  ...
  return Object.freeze({ version: 1, referenceCount: records.length,
    classifications: Object.freeze(classifications), blockers,
    cutoverReady: blockers === 0, downloadsPerformed: 0, containsFileContent: false });
}
