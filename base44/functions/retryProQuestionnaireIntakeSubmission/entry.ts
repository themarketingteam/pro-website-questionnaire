...
Deno.serve(async (req) => {
  try {
    ...
    const payloadSessionId =
      (typeof metadata.questionnaire_session_id === 'string' ? metadata.questionnaire_session_id : '') ||
      (typeof metadata.questionnaireSessionId === 'string' ? metadata.questionnaireSessionId : '') ||
      (typeof transformedPayload?.questionnaire_session_id === 'string' ? transformedPayload.questionnaire_session_id : '') ||
      questionnaireSessionId ||
      intake?.questionnaire_session_id ||
      '';
    ...
    if (intake?.linked_submission_id && forceRetry !== true) {
      return Response.json({
        success: true,
        alreadySubmitted: true,
        linkedSubmissionId: intake.linked_submission_id,
    ...
    if (payloadSessionId) {
      const existingSubmissions = await base44.asServiceRole.entities.ProFormSubmission.filter({
        'metadata.questionnaire_session_id': payloadSessionId
      });

      const existingSubmission = Array.isArray(existingSubmissions) && existingSubmissions.length > 0
      ...
      if (existingSubmission?.id) {
        await base44.asServiceRole.entities.ProFormSubmissionIntake.update(intake.id, {
          ...
          last_retry_at: new Date().toISOString(),
          retry_count: incrementRetryCount(intake.retry_count),
          notes: 'Retry linked to an existing ProFormSubmission for this questionnaire session instead of creating a duplicate.'
        });
