...
The browser grant vault creates/reuses a random per-browser device identifier. `deviceBindingHash` binds the grant to that random identifier and is not derived from invasive fingerprinting inputs. **Forget This Device** removes both the local grant and random device identifier. Browser-storage clearing has the same local effect. IndexedDB is preferred, localStorage is the durable fallback, and page memory is the nonpersistent fallback.
...
`verifyDraftRecoveryAccess` now authoritatively performs backend password verification, issuance, validation, forget-device audit, and rate/lockout enforcement. It uses the structured-token helper and no longer issues the legacy seven-day token. The existing admin page is intentionally not migrated in this batch; it must move to the new client/context and scoped backend draft APIs in later work before workflow certification.
...
5. migrate the legacy admin page to the implemented vault/context and scoped admin APIs, remove its old local-storage compatibility key, and keep admin authorization separate from user authentication;
...
The structured-token helper itself performs no I/O. The password-only admin authorization layer now supplies the endpoint and browser grant consumer described in the [admin contract](../admin/password-only-admin-recovery-authorization-contract.md), but is not mounted into the admin page and was not deployed or configured by this source batch.
