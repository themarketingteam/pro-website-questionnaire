...
| `BC-LOCAL-001` | `DR-LOCAL-002` | `storage-isolation.baseline-characterization.test.js` | Compare Redux persistence keys for synthetic Client A and B URLs. | Historical baseline used global `persist:pro-questionnaire-root`. | Use a non-secret client-scoped namespace. | Inverted | Pass | Feature branch produces distinct version 4 hashed Redux keys with no raw fixture identity. |
| `BC-LOCAL-002` | `DR-LOCAL-002` | same | Compare questionnaire session key/ID for Client A and B. | Historical baseline reused global `pro_questionnaire_session_id`. | Scope identity safely so clients cannot share a draft session. | Inverted | Pass | Feature branch produces distinct version 4 session keys and opaque IDs. |
| `BC-LOCAL-003` | `DR-LOCAL-001`, `DR-LOCAL-002` | same | Persist responses and approved credentials. | Historical baseline persisted responses and omitted credentials under one global root. | Persist approved form state only inside the isolated hashed client namespace. | Inverted | Pass | Version 4 scoped root contains the response and allowlisted credentials; `BC-LOCAL-004` proves another namespace cannot hydrate either. |
| `BC-LOCAL-004` | `DR-LOCAL-002` | same | Rehydrate Client A browser state under Client B URL. | Historical baseline hydrated Client A response for Client B. | Reject/quarantine cross-client local state. | Inverted | Pass | Client B remains empty while Client A rehydrates only from Client A's namespace. |
...
| `BC-LOCAL-005` | `DR-LOCAL-001`, `DR-LOCAL-002` | same | Write a failed-save backup and inspect read/key scope. | Historical backup was globally session-keyed and had no safe reader. | Write recoverable, safely scoped backups and expose controlled exact-scope reads. | Inverted | Pass | Version 4 backup contains approved metadata/state; exact namespace reads it and another namespace cannot. Automatic hydration remains deferred. |
