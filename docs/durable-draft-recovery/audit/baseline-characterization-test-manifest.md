...
| `BC-BOOT-002` | `DR-BOOT-001`, `DR-BOOT-002` | same | `localStorage` property getter throws `SecurityError`. | Module import rejects with `SecurityError`. | Bootstrap/render succeeds through guarded storage access within the time bound. | Invert | Pass | Rejected import captured after module reset. |
| `BC-BOOT-003` | `DR-BOOT-001`, `DR-BOOT-002` | same | `localStorage.getItem` throws. | Module import rejects. | Bootstrap/render succeeds with a safe fallback. | Invert | Pass | Synthetic getter failure propagated. |
| `BC-BOOT-004` | `DR-BOOT-001`, `DR-BOOT-002` | same | `localStorage.setItem` throws `QuotaExceededError`. | Module import rejects. | Bootstrap/render succeeds while reporting degraded persistence. | Invert | Pass | Synthetic quota failure propagated. |
