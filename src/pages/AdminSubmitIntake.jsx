...
export default function AdminSubmitIntake() {
  ...
  const removeComments = (s) => s.replace(/\/\*[\s\S]*?\*\//g, '').replace(/(^|\s)\/\/.*$/gm, '');
  const removeTrailingCommas = (s) => s.replace(/,\s*([}\]])/g, '$1');
  const balanceBrackets = (s) => {
    const oc = (s.match(/{/g) || []).length, cc = (s.match(/}/g) || []).length;
    const os = (s.match(/\[/g) || []).length, cs = (s.match(/\]/g) || []).length;
    let out = s;
    if (oc > cc) out += '}'.repeat(oc - cc);
    if (os > cs) out += ']'.repeat(os - cs);
    return out;
  };
  const attemptParseWithFixes = (raw) => {
    let t = raw;
    try { return JSON.parse(t); } catch {}
    const steps = [normalizeSmartQuotes, removeComments, removeTrailingCommas, balanceBrackets];
  ...
  const handleSaveJson = () => {
    setSaveError('');
    const parsed = (() => { try { return JSON.parse(rawJson); } catch { return attemptParseWithFixes(rawJson); } })();
    if (parsed) {
      setPayload(parsed);
      setEditing(false);
      setOriginalBeforeEdit(parsed);
      toast.success('JSON updated');
    } else {
      setSaveError('Invalid JSON. Could not auto-fix. Please correct the structure.');
      toast.error('JSON is invalid — please fix formatting (quotes, commas, or brackets).');
    }
  };
